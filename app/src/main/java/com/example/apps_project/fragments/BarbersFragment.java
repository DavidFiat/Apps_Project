...
public class BarbersFragment extends Fragment implements  BarbershopsAdapter.OnListBarbers{
    ...
    private BarbershopsAdapter barbershopsAdapter;
    private BarbersAdapter barbersAdapter;

    public BarbersFragment() {

        barbershopsAdapter = new BarbershopsAdapter();
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        barbershopsAdapter.setOnListBarbers(this);
        getBarbershops();
        getBarbers();
        return view;
    }
    ...
    private void getBarbershops() {
        FirebaseFirestore.getInstance().collection("barbershops").get().addOnCompleteListener(
                task ->{
                    barbershopsAdapter.clear();
                    for(DocumentSnapshot doc : task.getResult()){
                        Barbershop barber = doc.toObject(Barbershop.class);
                        barbershopsAdapter.addBarbershop(barber);
                    }
                }
        );
    }
